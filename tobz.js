...
module.exports = tobz = async (tobz, message) => {
    try {
                ...
                function isBadwordMsg(id){
                    ...
                    for (let i of msgBadword){
                        if(i.id === id){
                            ...
                            if (msg >= 1) {
                                kasar === true 
                                tobz.reply(from, '*[ANTI-BADWORD]*\nPeringatan Jika kamu 10x berkata kasar, kamu akan di kick otomatis oleh bot!', id).then(() => {
                                })
                            if (msg >= 12) {
                                kasar === true 
        ...
        // ANTI BADWORD | GIMENZ
        if(!isCmd && isKasar && isGroupMsg && isDetectorBadword && !isGroupAdmins && !isAdmin && !isOwner) { 
